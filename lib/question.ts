...
const getRandomQuestionShell = async (): Promise<{
  type: QuestionType;
  ...
  if (savedQuestionCategory === 'all') {
    ...
    if (questionType === QuestionType.Coding) {
      ...
      questionCategory =
        allCodingCategories[Math.floor(Math.random() * allCodingCategories.length)];
      ...
      questionCategory =
        allQuestionCategories[Math.floor(Math.random() * allQuestionCategories.length)];
    } else {
...
export function getCategoriesForType(type: QuestionType): string[] {
  const standard =
    type === QuestionType.Coding ? Object.values(CodingCategory) : Object.values(QuestionCategory);

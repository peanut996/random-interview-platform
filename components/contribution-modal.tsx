...
export function ContributionModal({ isOpen, onOpenChange }: ContributionModalProps) {
  ...
  const handleSubmitContribution = async () => {
    const selectedQuestions = parsedQuestions
      .filter((q) => q.selected)
      .map(({ selected, newCategory, showCategoryInput, editingTitle, ...questionData }) => {
        // Only keep the required fields for submission
        return questionData;
      });
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to submit contribution: ${response.status} ${response.statusText}`
        );
      }
  ...
  // Toggle question selection status
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
          ...
          <div className="flex flex-col flex-1 overflow-hidden">
            <h3 className="text-lg font-medium mb-3">
              ...
              <span className="text-sm font-normal text-muted-foreground ml-2">
                ({parsedQuestions.filter(q => q.selected).length}/{parsedQuestions.length} {t('contribution.selected')})
              </span>
            </h3>
        ...
        <DialogFooter className={`mt-4 pt-3 border-t ${parsedQuestions.length === 0 && !prUrl ? 'hidden' : ''}`}>
          {prUrl ? (
            <Button
              asChild

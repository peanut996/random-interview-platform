...
export default function Page() {
      ...
      if (currentQuestion) {
        setQuestionHistory(prevHistory => {
          const updatedHistory = prevHistory.map(item =>
            item.id === currentQuestion.id ? { ...item, answered: true } : item
          );
          localStorage.setItem('questionHistory', JSON.stringify(updatedHistory));
          return updatedHistory;
        });
  ...
  const handleConfirmation = async () => {
    ...
    try {
      ...
      await getModelAnswer(
        ...
        editorLanguage
      ); // Pass the editor language as a hint
  ...
  const loadQuestionFromHistory = (historyItem: QuestionHistory) => {
    ...
    if (historyItem.question.type === QuestionType.Coding && editorLanguage !== 'java') {
      ...
      toast({
        ...
        description:
          t('toast.switchedToCode.description') || 'The question type has been changed to coding',
        duration: 3000,
      });
    }
  };
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      {showSettingsModal && (
        <SettingsModal language={language} onClose={onCloseSettings} onReload={onNextQuestion} />
      )}
